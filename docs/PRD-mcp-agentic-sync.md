...
| **WP3** | Sync engine: local indexer + diff/plan | — (independent of WP1/WP2; can start in parallel) | Pure, testable, no I/O side effects beyond reading local files and calling `drive_list`. Hash-cache scope decision is explicit, not implicit (§6.3). |
