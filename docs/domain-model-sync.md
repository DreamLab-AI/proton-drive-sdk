...
├── folder: Option<NodeUid>              // subtree root this snapshot covers, if known
├── entries: Map<RelativePath, RemoteEntry>
├── taken_at_unix: Option<u64>           // unix epoch seconds, if recorded
...
├── node_uid: NodeUid
├── revision_uid: String                 // active revision's uid at snapshot time; the staleness token (§4 invariant 2)
├── sha1_hex: Option<ContentHash>         // recovered from decrypted xattr; None if xattr absent/unparseable
└── size: u64
```
...
├── ops: Vec<SyncOp>                     // ordered parent-before-child; order is significant (§4)
├── dirs_to_create: Vec<RelativePath>    // remote directories to create before the plan's Upload ops run
├── issued_at: SystemTime
└── superseded: Option<SyncPlanId>       // set by `resolve`; the plan this one replaces (§4 invariant 5)
```

```
SyncOp (entity, ordered within SyncPlan; identity = position + target path)
├── path: RelativePath
├── kind: Upload | UploadRevision | Download | Skip | Conflict(ConflictReason)
├── local: Option<LocalRef { content_hash, size, mtime }>              // snapshot-time facts, not live re-reads
├── remote: Option<RemoteRef { node_uid, revision_uid: String, content_hash: Option<ContentHash> }>
└── outcome: Option<ConflictOutcome>     // set on the resolved op after resolution
```

A `SyncPlan` is **immutable once issued.** Resolving a `Conflict` op does not
mutate the plan in place: `SyncPlan::resolve` produces a new, `Approved`
`SyncPlan` carrying the caller's `ConflictOutcome` choices, whose `superseded`
field records the original plan's id. The plan does not itself store a
reference back to the `LocalIndex`/`RemoteSnapshot` pair it was computed from
(there is no `computed_from` field); that pairing lives only in the call to
the pure `diff(local, remote) -> SyncPlan` function (`proton-drive-sync::diff`)
that produced it. This mirrors the "Draft → Active, never mutate in place"
shape already used for `Revision` in `domain-model-mvp.md`: plans are
versioned forward, not edited.
...
### ConflictReason (value object)

```
ConflictReason = ContentDiverged | NoRemoteDigest
```

The payload carried by `SyncOpKind::Conflict`, recording *why* a path was
flagged rather than transferred: `ContentDiverged` means both sides carry a
digest and they differ; `NoRemoteDigest` means the remote side has no
recoverable digest at all (xattr absent or unparseable), so no safe comparison
is possible (§4 invariant 3 forbids transferring in that case regardless).
...
2. **Staleness is detected at apply time via revision-uid mismatch, and an op
   ...
   it against the op's `remote.revision_uid`. A mismatch means the
...
3. **Hash equality short-circuits all transfer.** If both `local` and
   `remote` carry a `ContentHash` and they are equal, the planner emits
...
A stale `RemoteSnapshot` must not be fed into `diff()` to compute a new
